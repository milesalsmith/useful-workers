...
async function handleRequest(request) {
  ...
  // Check if the X-Known-Bot header is present and its value equals to 
'True'
  if (knownBotHeader === 'True') {
    // Send the request to electricjellyfish.org and return a 200 response 
with application/json content type
    ...
    if (botScoreHeader) {
      ...
      if (botScoreHeader > 30) {
        // Send the request to electricjellyfish.org and return a 200 
response with plain text content type
      ...
      // If neither header is present, send request directly to 
electricjellyfish.org
